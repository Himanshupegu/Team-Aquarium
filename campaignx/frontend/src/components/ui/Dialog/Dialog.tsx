                ...
                {isOpen && (
                    <DialogPrimitive.Portal forceMount>
                        ...
                        <DialogPrimitive.Content asChild>
                            <motion.div
                                initial={position === 'right' ? { x: '100%' } : { opacity: 0, y: 10 }}
                                animate={position === 'right' ? { x: 0 } : { opacity: 1, y: 0 }}
                                exit={position === 'right' ? { x: '100%' } : { opacity: 0, y: 10 }}
...
export const DialogFooter = ({
    ...
    <div className={`p-6 border-t border-gray-200 flex justify-end gap-4 ${className}`}>
        {children}
    </div>
);
